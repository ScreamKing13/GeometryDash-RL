    ...
    def test(self):
        for _ in range(10):
            ...
            while not done:
                ...
                # image = (state[:, :, 0] * 0.1 + state[:, :, 1] * 0.2 + state[:, :, 2] * 0.3 + state[:, :, 3] * 0.4).astype(np.uint8)
                # image = [cv2.Canny(next_frame_g, threshold1=i, threshold2=(i * 2)) for i in range(90, 101)]
                # image.extend([cv2.Canny(next_frame_g, threshold1=i, threshold2=(i * 3)) for i in range(90, 101)])
                # image = np.sum(image, axis=0) / len(image)
            ...
            # output = cv2.VideoWriter(f'test_{_}.avi',
            #                          cv2.VideoWriter_fourcc(*'DIVX'), float(fr / ex_time), (width, height))
            # for frame in frames:
